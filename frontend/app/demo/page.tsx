    ...
    return (
        ...
        >
            ...
            <div className="text-center px-4 mt-2">
                <p className="text-white font-semibold mb-[0.25rem] text-sm sm:text-base">
            ...
            <button type="button" className="btn-secondary px-[1.5rem] py-[0.5rem] rounded-lg text-sm pointer-events-none mb-2">
                Browse Files
            </button>
        </div>
    );
...
function Spinner({ phase }: { phase: Phase }) {
    ...
    return (
        <div className="flex flex-col items-center gap-[1.5rem] sm:gap-[2rem] py-[2.5rem] sm:py-[3rem]">
            <div className="relative w-[5rem] h-[5rem] sm:w-[6rem] sm:h-[6rem]">
            ...
            <div className="text-center px-4">
                <p className="text-white font-semibold text-base sm:text-lg mb-[0.25rem]">{STATUS_MESSAGES[phase]}</p>
            ...
            <div className="flex gap-[1rem] sm:gap-[1.5rem]">
                {steps.map((s, i) => (
                    <div key={s} className="flex flex-col items-center gap-[0.5rem]">
                        <div className={`w-[0.625rem] h-[0.625rem] rounded-full transition-all duration-500 ${steps.indexOf(phase) >= i ? "bg-white shadow-[0_0_8px_rgba(255,255,255,0.5)]" : "bg-white/15"}`} />
        ...
        <div className="space-y-[1.25rem]">
            ...
            <div className="flex items-start justify-between gap-[0.75rem]">
                <div className="min-w-0">
                    ...
                    <p className="text-white/30 text-xs font-mono mt-[0.125rem] truncate">{filename}</p>
                </div>
                <button
                    onClick={onReset}
                    className="flex items-center gap-[0.375rem] text-white/35 hover:text-white transition-colors text-xs sm:text-sm border border-white/10 px-[0.625rem] py-[0.375rem] rounded-lg hover:border-white/20 shrink-0"
                >
                    <X size={13} /> New Scan
                </button>
            </div>
    ...
    return (
        <div className="min-h-screen bg-black pt-[9rem] sm:pt-[10rem] pb-[4rem] w-full flex flex-col items-center">
            <div className="w-full max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                {/* Page header */}
                <div className="text-center mb-[2rem] sm:mb-[3rem]">
                    <span className="status-badge mb-[1rem] inline-flex">
                    ...
                    <p className="text-white/40 text-center w-full text-sm sm:text-base leading-relaxed">
                        Upload a FLAIR MRI scan and watch our U-Net model detect and highlight the tumor region in real time.
                    </p>
                </div>
                ...
                <div className="glass-strong rounded-2xl sm:rounded-3xl p-5 sm:p-8 border border-white/8">
                    ...
                    {phase === "error" && (
                        <div className="flex flex-col items-center gap-[1rem] py-[2.5rem] sm:py-[3rem]">
                            <div className="w-[3.5rem] h-[3.5rem] sm:w-[4rem] sm:h-[4rem] rounded-2xl border border-red-500/20 bg-red-500/8 flex items-center justify-center">
                                <AlertCircle size={24} className="text-red-400" />
                            </div>
                            <p className="text-white font-semibold">Something went wrong</p>
                            <p className="text-white/30 text-sm text-center">Please try again with a valid MRI image.</p>
                            <button onClick={handleReset} className="btn-secondary px-[1.5rem] py-[0.625rem] rounded-lg text-sm mt-[0.25rem]">Try Again</button>
                        </div>
                    )}
                </div>
                ...
                {/* Info cards */}
                {phase === "idle" && (
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-[1rem] mt-[1.25rem] sm:mt-[2rem]">
                        {[
                            ...
                            <div key={title} className="glass rounded-xl p-[1.25rem] border border-white/8 h-full flex flex-col justify-center">
                                <p className="text-white/50 text-[10px] sm:text-xs font-semibold uppercase tracking-wider mb-[0.5rem]">{title}</p>
                                <p className="text-white/40 text-xs sm:text-sm leading-relaxed">{info}</p>
                            </div>
                        ))}
                    </div>
                )}
